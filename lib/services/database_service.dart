    ...
    return openDatabase(
      ...
      onUpgrade: (Database db, int oldVersion, int newVersion) async {
        if (oldVersion < 2) {
          ...
          await db.execute('ALTER TABLE $_ebooksTable ADD COLUMN grade TEXT NOT NULL DEFAULT "1"');
        }
        if (oldVersion < 3) {
          // Add category and coverImagePath columns
          try {
            await db.execute('ALTER TABLE $_ebooksTable ADD COLUMN category TEXT NOT NULL DEFAULT "Textbooks"');
          } catch (e) {
            // Column might already exist
          }
          try {
            await db.execute('ALTER TABLE $_ebooksTable ADD COLUMN coverImagePath TEXT');
          } catch (e) {
            // Column might already exist
          }
        }
