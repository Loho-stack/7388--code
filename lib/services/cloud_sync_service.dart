...
class CloudSyncService {
  ...
  Future<List<Ebook>> getAvailableCloudBooks() async {
    try {
      ...
      return cloudBooks.where((book) => !localBookIds.contains(book.id)).toList();
    } catch (e) {
  ...
  Future<bool> downloadBook(
    ...
    try {
      ...
      final downloadUrl = _convertToDirectDownloadLink(cloudBook.serverUrl ?? '');
      print('Download URL: $downloadUrl');
      
      // Sanitize filename
      final safeFileName = cloudBook.title.replaceAll(RegExp(r'[^\w\s-]'), '').trim();
      ...
      if (cloudBook.coverImagePath != null && cloudBook.coverImagePath!.isNotEmpty) {
        try {
          print('Downloading thumbnail: ${cloudBook.coverImagePath}');
          final thumbnailUrl = _convertToDirectDownloadLink(cloudBook.coverImagePath!);
  ...
  Future<bool> deleteDownloadedBook(String bookId) async {
    try {
      ...
      try {
        await _databaseService.deleteEbook(bookId);
        print('Removed orphaned database entry: $bookId');
        return true;
      } catch (dbError) {
        print('Failed to remove from database: $dbError');
        return false;
      }
    }
  }
  ...
  Future<Map<String, dynamic>> getBookStatus(String bookId) async {
    ...
    if (isDownloaded) {
      final localBooks = await _databaseService.getAllEbooks();
      final book = localBooks.firstWhere((b) => b.id == bookId);
      return {
        'downloaded': true,
